...
if ($way == "invoiceprint") {
    ...
    $bookingdetails = $con->query("SELECT * FROM domestictourbookinghistory WHERE id='{$bookingid}'");
    ...
    if (strlen($bookingmethod) > 30) {
        ...
        if (preg_match('/\((\d+(\.\d+)?)\s*\+\s*(\d+(\.\d+)?)\)/', $bookingmethod, $matches)) {
            $topupwallet = $matches[1];     // e.g. 9
            $bonustp     = $matches[3];     // e.g. 0.5
        } else {
            $topupwallet = "error";
            $bonustp     = "error";
        }
            ...
            <td style="width:100%">
                <div style="width:100%">
                    <table style="width:100%">
                        <tr style="width:100%">
                            <td style="width:2%"></td>
                            <td style="width:96%;color:#000;font-size:12px" align="start">
