...
if ($values["status"] == "success") {
    ...
    if ($way == "login") {
        ...
        if (mysqli_num_rows($datares) == 1) {
            ...
            $availablewithdrwabalance = $con->query("SELECT * FROM availablewithdrwabalance WHERE user_id='{$values["userid"]}'");
            $awbcredit = 0;
            $awbdebit = 0;
            ...
            if (mysqli_num_rows($checkpay) == 0) {
                ...
                if (mysqli_num_rows($bankdetails) == 1) { {
                        $response["trc20_address"] = $getbankdetails["trc20_address"];
                        $response["ac_holdername"] = $getbankdetails["ac_holdername"];
                        $response["ac_bankname"] = $getbankdetails["ac_bankname"];
        ...
        $bankdetails = $con->query("SELECT * FROM userbankdetails WHERE user_id='{$values["userid"]}'");
        $getbankdetails = $bankdetails->fetch_assoc();
        $trc20address = $getbankdetails["trc20_address"];
        ...
        $bankdetails = $con->query("SELECT * FROM userbankdetails WHERE user_id='{$values["userid"]}'");
        $getbankdetails = $bankdetails->fetch_assoc();
        $ac_bankname = $getbankdetails["ac_bankname"];
        $ac_holdername = $getbankdetails["ac_holdername"];
        $ac_number = $getbankdetails["ac_number"];
        $branch = $getbankdetails["branch"];
        $ifsc_code = $getbankdetails["ifsc_code"];
                            ...
                            <td align="center" bgcolor="#ffffff">
                                <div style="width: 100%; max-width: 600px; background-color:white; display: table;">
                                    ...
                                    <div align="start">
                                        <p><b>Withdraw Details:</b><br>
                                            Type&nbsp;:&nbsp;Bank<br>
                                            Account Holder Name&nbsp;:&nbsp;' . $ac_holdername . '<br>
                                            Account Bank Name;:&nbsp;' . $ac_bankname . '<br>
