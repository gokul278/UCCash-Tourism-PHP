...
if ($values["status"] == "success") {
    ...
    if ($way == "login") {
        ...
        $data = $con->query("SELECT * FROM domestictourdestination WHERE id='{$bookingid}'");
        $getdata = $data->fetch_assoc();
        ...
        $tourdetails = $con->query("SELECT * FROM domestictourdestination WHERE id='{$bookingid}'");
        $gettourdetails = $tourdetails->fetch_assoc();
        if ($type == "bonustravelpoints") {
            ...
            $fintopupwalletpoints = isset($_POST["topupwalletpoint"]) ? (float) $_POST["topupwalletpoint"] : 0;
            $finbonustravelpoints = isset($_POST["bonustravelpoints"]) ? (float) $_POST["bonustravelpoints"] : 0;
            $total = $fintopupwalletpoints + $finbonustravelpoints;
            $datasql = "SELECT * FROM userdetails WHERE user_id='{$values["userid"]}'";
            $datares = $con->query($datasql);
            $datarow = $datares->fetch_assoc();
            ...
            $content .= '
            Type&nbsp;:&nbsp;Top-up Wallet + Bonus Travel Points<br>
            Person&nbsp;:&nbsp;' . $personinput . '<br>
        ...
        $tour = $con->query("SELECT * FROM domestictourdestination WHERE id='{$bookingid}'");
        ...
        // First, try to update
        $con->query("UPDATE userbankdetails SET otp='$otp' WHERE user_id='{$values["userid"]}'");

        // Now check if the row exists
        $result = $con->query("SELECT * FROM userbankdetails WHERE user_id='{$values["userid"]}'");
        $getUpdateOTP = $result->fetch_assoc();

        // If no row exists, insert a new one
        if (!$getUpdateOTP) {
            $con->query("INSERT INTO userbankdetails (user_id, otp) VALUES ('{$values["userid"]}', '$otp')");
        }
                ...
                if ($bonustpbalance >= $bonustravelpoints) {
                    ...
                    if ($netamount == $totalpriceuser) {
                        ...
                        $query = "INSERT INTO domestictourbookinghistory (
                                ...
                                '$tour_fromdate',
                                '$tour_todate',
                                'Top-up Wallet Point, Bonus Travel Point <br> ($st_escaped + $bt_escaped)',
                                '$gstamount_escaped',
                                '$netamount',
                                'booked'
                            )";
