$(document).ready(() => {
    $.ajax({
        type: "POST",
        url: "./requiredFiles/ajax/domestictourdescriptionAjax.php",
        data: {
            "way": "login"
        },
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (response.status == "success") {
                ...
                $("#tour_amount").html('<b>Amount : '+response.tour_amount+' TP per person (Included GST 5%)</b>')
                ...
                $("#confirmbtn").html('<a href="domesticconfirmpage.php?bookingid='+response.tourid+'"><button style="background-color: #f7c128; color: white; padding: 10px 20px; border: none; border-radius: 5px; font-size: 16px; cursor: pointer; animation: pulse 1.5s infinite;">Continue Booking</button> </a>')
