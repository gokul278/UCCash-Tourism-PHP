    ...
    success: function (res) {
      ...
      if (response.status === "success") {
        ...
        if (response.roleId === "1") {
          ...
          location.replace("travel coupon approval.php");
        } else if (response.roleId === "3") {
          location.replace("withdraw approval.php");
        } else if (response.roleId === "4") {
          location.replace("tour destination edit.php");
        }
        console.log(response);
      } else if (response.status === "failed") {
        $("#loginbtn").html("Login");
        $("#errormsg").html(
          '<p style="padding:15px">' + response.message + "</p>"
        );
      }
    },
  });
