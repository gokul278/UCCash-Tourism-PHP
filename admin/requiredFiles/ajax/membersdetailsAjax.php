...
if ($values["status"] == "success") {
    ...
    if ($way == "login") {
        ...
        $filtersDate = $con->query("SELECT * FROM idactivation WHERE created_at >= '{$_POST["fromDate"]} 00:00:00' AND created_at <= '{$_POST["toDate"]} 23:59:59';");

        $tabledata = "";
        foreach ($filtersDate as $filterIndex => $filterDate) {
            // echo $filterDate["user_id"];

            $data = $con->query("SELECT * FROM userdetails WHERE user_id='{$filterDate["user_id"]}'");
        ...
        $response["status"] = "success";
        echo json_encode($response);
    } else if ($way == "typeFilter") {

        $activationType = $_POST["typevalue"] == "Member" ? "notactivated" : "activated";

        $data = $con->query("SELECT * FROM userdetails WHERE user_referalStatus = '{$activationType}'");
        ...
        foreach ($data as $index => $getdata) {
            ...
            if ($getdata["user_id"] != "UCT99999") {
                $tabledata .= '
            <tr>
                <td>' . ($activationType == "notactivated" ? $index + 1 : $index) . '</td>
                <td>' . $getdata["user_id"] . '</td>
                <td>' . $getdata["user_name"] . '</td>
                <td>' . $getdata["user_email"] . '</td>
                <td>' . $getdata["user_phoneno"] . '</td>
