...
if ($values["status"] == "success") {
    ...
    if ($way == "login") {
        ...
        $lastimage = $con->query("SELECT bankdeposit_image FROM monthlysavings WHERE id=1");
        ...
        if (unlink("../.././img/monthly/" . $getlastimage["bankdeposit_image"])) {

            if (move_uploaded_file($_FILES["updateimage"]["tmp_name"], "../.././img/monthly/" . $newImageName)) {

                $insertimgsql = "UPDATE monthlysavings SET bankdeposit_image ='{$newImageName}' WHERE id=1";
