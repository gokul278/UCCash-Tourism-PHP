...
if ($values["status"] == "success") {
    ...
    if ($way == "login") {
        ...
        $pendinginvoice = $con->query("UPDATE monthlysavingpendinginvoice SET action='pending', remark='{$reason}' WHERE id='{$invoiceid}' AND  user_id='{$userid}'");

        if ($pendinginvoice) {
            $savinghistory = $con->query("UPDATE monthlytpsavinghistory SET action='reject', remark='{$reason}' WHERE id='{$id}' AND  user_id='{$userid}'");
            if ($savinghistory) {
                $response["status"] = "success";
                echo json_encode($response);
            }
